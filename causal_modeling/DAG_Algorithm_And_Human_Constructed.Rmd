...
- We use a constraint-based learning algorithm to generate a DAG on bnlearn. 
- We modify this generated DAG to reflect our domain knowledge about the assumed interactions between variables. 
- We visualize a structural comparison of the original and new DAGS. 
- Finally, we compare the global Markov property of our prior constructed DAG (based solely upon domain knowledge and intuition) versus this new DAG, constructed from a hybrid of human and algorithmic signals.
...
among nodes, then set the directions of all directed arcs. The algorithm then set futher arc directions mandated by our specified whitelist and blacklist node relationships, and finally marked final v-structures and set final arc directions before stopping.
...
1. In the PDAG, there is are undirected edge between all three variables in the CC (county COVID-19 cases) node's Markov blanket: SPL (state political leaning),
...
Our justification for this claim is that the voting age is 18, and few individuals earn high school diplomas after reaching voting age. State political leaders
are determined by voters.
...
In this visual comparison of the domain-knowledge DAG and hybrid-knowledge DAG, there is a code for the meaning of each edge type:
...
causal relationship from bans on large gatherings to county case rate between the two DAGs.
